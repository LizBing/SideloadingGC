...
size_t EpsilonArguments::conservative_max_heap_alignment() {
  if (EpsilonSideloading)
    return sideloadingArguments.conservative_max_heap_alignment();
...
void EpsilonArguments::initialize() {
  if (EpsilonSideloading) {
    sideloadingArguments.initialize();
    return;
  }
...
void EpsilonArguments::initialize_alignments() {
  if (EpsilonSideloading) {
    sideloadingArguments.initialize_alignments();
    return;
  }
...
CollectedHeap* EpsilonArguments::create_heap() {
  if (EpsilonSideloading)
    return sideloadingArguments.create_heap();
